...
const Disposables = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['Disposables'],
    queryFn: productService.getDisposables,
  });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => productService.updateDisposableBase(id, data),
  });
  ...
  const toggleBaseMutation = useMutation({
    mutationFn: ({ id, isActive }) => {
      ...
      return productService.updateDisposableBase(id, formData);
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['Disposables']);
    },
  ...
  const deleteBaseMutation = useMutation({
    mutationFn: productService.deleteDisposableBase,
    onSuccess: () => {
      queryClient.invalidateQueries(['Disposables']);
  ...
  const handleCreate = async () => {
    const shouldCreate = await confirm({
      ...
      message: 'Add this Disposable to the database?',
      confirmText: 'Create',
      cancelText: 'Cancel',
    });
    ...
    try {
      ...
      queryClient.invalidateQueries(['Disposables']);
      toast({ title: 'Disposable created', status: 'success' });
      onClose();
      navigate('/products/Disposables', { replace: true });
    } catch (error) {
      toast({
        title: 'Error',
        description: error?.message || 'Failed to create Disposable',
        status: 'error',
      });
    }
  };
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: error?.message || 'Failed to update Disposable',
        status: 'error',
      });
    }
  };
    ...
    <Box p={4}>
      <VStack spacing={4} align="stretch">
        <HStack justify="space-between">
          <Text fontSize="2xl" fontWeight="bold">Disposable</Text>
        ...
        {products.length === 0 ? (
          <Center h="200px">
            <Text color="gray.500">No Disposable products</Text>
          </Center>
        ) : (
      ...
      <Modal isOpen={isOpen} onClose={onClose} size="full">
        ...
        <ModalContent bg={colorMode === 'dark' ? 'gray.800' : 'white'}>
          <ModalHeader>Add Disposable</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
      ...
      <Modal isOpen={isEditOpen} onClose={onEditClose} size="lg">
        <ModalOverlay />
        <ModalContent bg={colorMode === 'dark' ? 'gray.800' : 'white'}>
          <ModalHeader>Edit Disposable</ModalHeader>
          <ModalCloseButton />
